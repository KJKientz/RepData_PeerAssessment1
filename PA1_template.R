...
##alternative using the dplyr package, but this will bring back NAs AND it requires using the group_by function 
# library(dplyr)
# day <- group_By(activitydata, date)
# steps_day <- (summarize(day,total=sum(steps)))
...
##alternative approach to calculating the max
#max_step_avg_row <- which.max(steps_interval$steps)
#steps_interval[max_step_avg_row,]$steps
...
##activity_filled <- activitydata
##steps_vector <- activity_filled$steps
##steps_vector[which(is.na(steps_vector))] <- mean(activitydata$steps,na.rm=TRUE)
##activity_filled$steps <- steps_vector
